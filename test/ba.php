...
function test_get_topics_list($users) {
  ...
  foreach ($users as $phone_num => $sid) {
    $ret = json_decode(file_get_contents("http://{$host}/api/ba/topics.php?ba_id=1&p={$p}", false, $get_timeout), true);
    if ((int)$ret['code'] == 0) {
...
function test_comment_topic() {
  ...
  foreach ($login_ok_users as $phone_num => $sid) {
    $topic_id = $add_ok_topic_list[array_rand($add_ok_topic_list)];
    $ret = json_decode(util::post_data("http://{$host}/api/ba/comment.php",
                                       array("sid" => "{$sid}",
                                             "topic_id" => $topic_id,
    ...
    if ((int)$ret['code'] == 0) {
      ...
      printf("%s comment_topic failed [%s]\n", $phone_num, $ret['desc']);
    }
  }
  return $comment_ok;
}
$comment_ok_num = 0;
$begin_time = microtime(true);
$comment_ok_num = test_comment_topic();
$diff = round(microtime(true) - $begin_time, 3) + 0.001;
printf("comment_topic ok %d %s (%s req/sec)\n", $comment_ok_num, $diff, round($comment_ok_num/$diff, 2));
