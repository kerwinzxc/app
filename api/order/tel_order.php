...
do {
  ...
  if ($act == 'get_list') { // get list
    $total_num = tb_tel_ask_order::query_order_num($user_id);
    $order_list = array();
    $page = 1;
    if ($total_num > 0) {
      ...
      if (($page - 1) * ONE_PAGE_ITEMS <= $total_num
          && $page >= 1) {
        $order_list = tb_tel_ask_order::query_order_limit($user_id,
        ...
        $order_list = fn_tel_ask_order::build_order_brief_list($order_list);
      }
    ...
    if ($act == 'del') { // del
      $order_info = tb_tel_ask_order::query_order_by_id($order_id);
      if ($order_info === false) {
        $ret_code = ERR_DB_ERROR;
        break;
      ...
      if (tb_tel_ask_order::del_one($order_id) === false) {
